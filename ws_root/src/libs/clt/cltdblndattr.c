...
static const DblndAttr dblndAttrTable[DBLA_NoMore]= {
	{	/* Dbl_EnableAttrs */
	...
	if (dblendAttrList == NULL)
        {
            ...
            return -1;
#else
            return CLT_ERR_BADPTR;
#endif
        }

	if ( (dblndCmdList = dblendAttrList->data) == NULL){
		result = CLT_AllocSnippet(dblendAttrList, SIZEOF_DBLNDCMD_LIST);
		if (result < 0){
		    return result;
		} else {
			dblndCmdList = dblendAttrList->data;
			dblendAttrList->size = SIZEOF_DBLNDCMD_LIST;
			bzero (dblndCmdList, SIZEOF_DBLNDCMD_LIST*4);
