...
static const TxAttr texAttrTable[TXA_NoMore]= {
	{	/* Tx_MinFilter */
						   ...
						   uint32          	attrValue)
{
	...
	if ( (txCmdList = textureAttrList->data) == NULL){
		result = CLT_AllocSnippet(textureAttrList, SIZEOF_TXCMD_LIST);
		if (result < 0) {
			return result;
		} else {
			txCmdList = textureAttrList->data;
			textureAttrList->size = SIZEOF_TXCMD_LIST;
			bzero (txCmdList, SIZEOF_TXCMD_LIST*4);
