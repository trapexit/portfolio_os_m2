...
/*****************************************************************************
**
**  @(#) mpegaudiosubscriber.c 96/09/06 1.15
**
*****************************************************************************/
#include <string.h>
#include <stdlib.h>
 ...
 ********************************************/
static SAudioContextPtr InitializeMPEGAudioThread(SAudioCreationArgs *creationArgs);
...
static SAudioContextPtr InitializeMPEGAudioThread(SAudioCreationArgs *creationArgs)
{
	...
	for ( index = 0; index < SA_SUBS_MAX_CHANNELS; index++ )
		ctx->channel[index].numBuffers = creationArgs->numberOfBuffers;
...
/******************************************************************************
|||	AUTODOC -public -class streaming -group startup -name NewMPEGAudioSubscriber
...
|||
|||	    numberOfBuffers
|||	        Number of audio buffers to allocate for storing decompressed
|||	        data.
|||
|||	  Return Value
|||	
...
static void	MPEGAudioSubscriberThread( int32 notUsed, SAudioCreationArgs *creationArgs )
{
	...
	ctx = InitializeMPEGAudioThread(creationArgs);
	...
	/* Additional initialization that belongs in InitializeMPEGAudioThread(): */
	...
	/* Allocate decompressed audio buffers. */
