/******************************************************************************
**
**  @(#) sasoundspoolerinterface.c 96/08/29 1.24
**
*****************************************************************************/
	...
	if ( IsChanActive(chanPtr) )
		...
		while( ( sbn = ssplRequestBuffer( chanPtr->spooler ) ) != NULL )
			...
			if( subMsg != NULL )
				 ...
				 * this chunk */
				status = ssplSetBufferAddressLength( chanPtr->spooler, sbn,
					(char *)&audioData->samples, audioData->actualSampleSize );	
				CHECK_NEG("ssplSetBufferAddressLength", status);
		...
		if ( status > 0 )	/* one or more buffers completed */
		{
			...
			if ( ssplIsSpoolerActive(chanPtr->spooler) )
			{
				...
				if ( sbn != NULL )
				{
					...
					{
						SAudioDataChunkPtr	audioDataPtr =
							((SubscriberMsgPtr)msgPtr)->msg.data.buffer;
						time = audioDataPtr->time;
						branchNumber =
							((SubscriberMsgPtr)msgPtr)->msg.data.branchNumber;
					}
					else
					{
...
static Err	SpoolBufCompletionFunc( SoundSpooler *sspl,
	...
	switch ( ssplGetSBMsgClass(msg) )
		...
		case SSPL_SBMSGCLASS_END:
			...
			audioDataPtr = (SAudioDataChunkPtr) subMsg->msg.data.buffer;	
			...
			*(int32*) &audioDataPtr->samples = 0xdeadbeef;
