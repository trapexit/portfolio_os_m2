/******************************************************************************
**
**  @(#) mpegvideochannels.c 96/11/26 1.4
**
******************************************************************************/
 ...
 * called when a new header chunk is received on a given channel.  When the channel is
 * stopped, the initialized flag is cleared, so we'll re-init on the next header received.
 * However, if a stream is looped without being stopped, we'll ignore the header(s) that
 * come through on the second and subsequent loops.
 *******************************************************************************************/
Err InitMPEGVideoChannel(MPEGVideoContextPtr ctx, MPEGVideoHeaderChunkPtr headerPtr)
	{
	Err						status = kDSNoErr;
	MPEGVideoChannelPtr		chanPtr = ctx->channel;
			
	ADD_MPVD_TRACE_L2( MPEGVideoTraceBufPtr, kTraceChannelInit, 0, 0, 0 );
	...
	if ( !IsMPEGVideoChanInitialized(chanPtr) )
		...
		chanPtr->frameDurationTicks = MPEGTimestampToAudioTicks(chanPtr->frameDurationPTS);

		ctx->curVSize = headerPtr->maxPictureArea; 	/* dumb name: maxPictureArea is really vertical size */
		ctx->curHSize = (ctx->curVSize < 480) ? 352 : 704;	/* hSize follows vSize */

#if 0
		printf("Calling FMVSetVideoSize h=%d v=%d\n", ctx->curHSize, ctx->curVSize);
#endif

		status = FMVSetVideoSize(&ctx->fmvDevice, ctx->newframeBuffers.bitDepth, ctx->curHSize, ctx->curVSize, kCODEC_SQUARE_RESAMPLE);
		if (status >= 0) 
			{
			chanPtr->status	|= (MPVD_CHAN_INITIALIZED | CHAN_ENABLED);  
			}
		else
			{
			ERROR_RESULT_STATUS("FMVSetVideoSize() failed", status);
			}
...
Err StopMPEGVideoChannel(MPEGVideoContextPtr ctx, uint32 channelNumber)
	...
	chanPtr->status &= ~(CHAN_ACTIVE|MPVD_CHAN_INITIALIZED);
...
Err	ProcessNewMPEGVideoDataChunk(MPEGVideoContextPtr ctx, SubscriberMsgPtr subMsg)
	{
	Err						status = kDSNoErr;
	uint32					channelNumber = 0; /*<HPP> always 0 for WhiteBook */
	MPEGVideoChannelPtr		chanPtr;
