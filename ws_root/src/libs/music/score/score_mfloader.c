/* @(#) score_mfloader.c 96/02/23 1.14 */
/* $Id: score_mfloader.c,v 1.10 1995/03/14 23:58:54 peabody Exp $ */
/****************************************************************
**
...
static int32 PrepareScoreSequence( Sequence *SeqPtr, uint8 *EventDataPtr, int32 MaxEvents );
static int32 LoadScoreEvent( MIDIFileParser *mfpptr, const uint8 *Data, int32 NumBytes);
...
/* To avoid fragmantation, allocate space for smaller event memory now. */
	TempEvents = (uint8 *)AllocMem( DataSize, MEMTYPE_ANY | MEMTYPE_TRACKSIZE | MEMTYPE_FILL );
	if( TempEvents == NULL )
	{
	...
	Events = (uint8 *)AllocMem( DataSize, MEMTYPE_ANY | MEMTYPE_TRACKSIZE | MEMTYPE_FILL );
	if( Events == NULL )
	{
...
/* By freeing and reallocating, we avoid fragmentation. */
			FreeMem( TempEvents, -1 );
			DataSize = SeqPtr->jglr_Many * SeqPtr->seq_EventSize;
			TempEvents = (uint8 *)AllocMem( DataSize, MEMTYPE_ANY | MEMTYPE_TRACKSIZE );
			if( TempEvents )
			{
	...
	else
	{
		...
		FreeMem( TempEvents, -1 );  /* 931208 Fix leak. */
	}
...
******************************************************************/
/**
 |||	AUTODOC -public -class libmusic -group Score -name MFLoadSequence
 ...
 |||
 |||	    filename
 |||	        Pointer to a character string containing the
 |||	        name of the format 0 MIDI file from which to
 |||	        load the sequence.
 |||
 |||	    SeqPtr
 |||	        Pointer to a juggler sequence in which to
...
******************************************************************/
/**
 |||	AUTODOC -public -class libmusic -group Score -name MFLoadCollection
 ...
 |||
 |||	    ColPtr
 |||	        Pointer to a juggler collection in which to
...
******************************************************************/
/**
 |||	AUTODOC -public -class libmusic -group Score -name MFDefineCollection
 ...
 |||
**/
int32 MFDefineCollection ( MIDIFileParser *mfpptr, uint8 *Image, int32 NumBytes, Collection *ColPtr)
{
	int32 Result;
...
/******************************************************************/
/**
 |||	AUTODOC -public -class libmusic -group Score -name MFUnloadCollection
...
******************************************************************/
/**
 |||	AUTODOC -public -class libmusic -group Score -name InterpretMIDIEvent
...
DBUG(("InterpretMIDIEvent( 0x%x, 0x%x\n", SeqPtr, *(uint32 *) &MEvCur->mev_Command));
	...
	Result = InterpretMIDIMessage ( scon, (uint8 *) &MEvCur->mev_Command, IfMute);
	if (Result < 0) return Result;
	return 0;
}
