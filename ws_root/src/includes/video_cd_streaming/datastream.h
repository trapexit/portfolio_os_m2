...
**  
**  @(#) datastream.h 96/11/26 1.7
**
**	08/20/96  Ian	Added ResetDataAcq() function, kAcqOpReset opcode, and
**					'reset' fields in the DataAcqMsg structure.
**					Removed the old GetDataAcqBlockFile() function which the
**					VideoCD app no longer uses.
*****************************************************************************/
...
#define __STREAMING_DATASTREAM_H
	...
	#include <video_cd_streaming/dserror.h>
#endif
...
enum StreamOpcode {
	...
	kStreamOpHeaderInfo	= 12, 	/* mostly used to pass first subscriber header information */
	kStreamOpShuttle	= 13,	/* start for fast forward/reverse operation */
	kStreamOpPlay		= 14,	/* play the stream in a regular play mode */
	kStreamOpSetAction	= 15, 	/* tell subscriber to meet a condition and take an action */
	kStreamOpTotal				/* total number of Opcodes for subscribers */
	};
...
typedef struct SubscriberMsg {
	...
	union {
		struct {					/* kStreamOpData */
			...
			uint32	bufferSize;		/* size of our buffer */
			int32	pts;			/*  pts */
			int8	ptsValid;		/* if the above pts is valid */
			uint8	packetStart;	/* packet start flags */
		} data;
		...
		struct {					/* kStreamOpShuttleStart/Stop */
			int32	rate;			/* if (0) stop if > 0 forward else backward */
		} shuttle;
		struct {					/* kStreamOpSetAction */
			int32	type;			/* type of action to take */
			uint32	pts;			/* action time in PTS to occur */
			void*	proc;			/* action procedure to call */
			void*	param;			/* parameter to pass to the above action */
		} action;
		struct {
			uint32  audioModes;
			uint32	audioEmphasis;
	 	} audioParam;
	} msg;
...
enum DataAcqOpcode {
	...
	kAcqOpReset			= 5		/* Reset Whitebook block offset and count (IE, seek on disc) */
	};
...
typedef struct DataAcqMsg {
	...
	union {
		...
		struct {						/* kAcqOpReset */
			uint32			blockStart;	/* new psuedo-file start block on disc */
			uint32			blockCount;	/* count of blocks in the psuedo-file */
			uint32			cdromSpeed;	/* the 'x' speed (1x, 2x, 4x) to run the drive at */
			} reset;
		} msg;
...
enum DataAcqMarkerOptions {		/* msg.marker.value:  */
	...
	GOMARKER_RELATIVE	= 8,	/* value == relative number of blocks to skip */
...
enum DSRequestOpcode {
	...
	kDSOpShuttle			= 11,	/* start fast forward/reverse mode */
	kDSOpPlay				= 12, 	/* start regular play mode */
	kDSOpFrameDisplayed		= 13,	/* send when a frame is done */
	kDSOpGetMPEGTime 	 	= 14,	/* ask the stream to give us the time */
	kDSOpWaitStartOfStream	= 15,	/* register for SOS notification */
	kDSOpWaitTriggerBit		= 16, 	/* register for Trigger-Bit notification */
	kDSOpDoHighResStills	= 17,	/* turn high res stills processing on or off */
	kDSOpTotal						/* total number of Opcodes */
	};
...
typedef struct DSRequestMsg {
	...
	union {
		...
		struct {							/* kDSOpShuttle */
			int32 		rate;				/* the rate indicates if we are going forward */
											/* or reverse and by how much*/
		} shuttle;
		...
		struct {
			uint32		*timePtr;
			Boolean		*fromVideoPTSPtr;
		} getMPEGTime;

		struct {							/* kDSOpDoHighResStills */
			Boolean		option;				/* TRUE to return high-res stills, FALSE */
		} doHighResStills;					/* to return normal-res stills. */
	} msg;

	} DSRequestMsg, *DSRequestMsgPtr;

/******************************************************************************
 *	Data Acquistion private structures
 ******************************************************************************/

typedef struct AcqContext	AcqContext, *AcqContextPtr;
...
Err		ResetDataAcq(Item acqMsgPort, uint32 blockStart, uint32 blockCount, uint32 cdromSpeed);
