...
**  @(#) mpeg.h 96/02/20 1.5
**
**  Definitions for ISO 11172 MPEG-1 and ISO 13818 MPEG-2 structures,
**  including system (aka multiplex), audio, and video layers.
**
**************************************************************************/
...
/*********************************************************************
 * WhiteBook (VideoCD) File System definitions.
 *********************************************************************/
...
/*********************************************************************
 * ISO 11172-1 MPEG-1 System layer definitions.
 *********************************************************************/
...
#define ZERO_BYTE					((uint8)0)
#define STUFFING_BYTE				((uint8)0xFF)
...
/* Define this macro before including MPEG.h if you need the MPEG-2 PES definitions. */
#ifdef NEED_MPEG2_PES_DEFINITIONS
...
typedef struct PES_PacketPart8 {
	uint8		PES_private_data[16];	/* may not contain the sequence $00 00 01 */
...
#endif	/* #ifdef NEED_MPEG2_PES_DEFINITIONS */
...
typedef struct VideoSequenceHeader {
	...
	uint8			optional_intra_quantizer_matrix[64];	/* optional;
						* actually starts 1 bit earlier if present, and the;
						* last bit holds load_non_intra_quantizer_matrix */
	uint8			optional_non_intra_quantizer_matrix[64];	/* optional */
	/* optionally followed by sequence extension data */
	/* optionally followed by user data */
} VideoSequenceHeader, *VideoSequenceHeaderPtr,
	ISOVideoSequenceHeader, *ISOVideoSequenceHeaderPtr;
