...
#define __INTERNATIONAL_INTL_H


/******************************************************************************
**
**  @(#) intl.h 96/02/28 1.22
**
...
#define INTL_ERR_BADPRIV          MakeIErr(ER_SEVERE,ER_C_STND,ER_NotPrivileged)
...
/* Certain functions in Portfolio are defined as taking a normal (char *)
 * parameter, but are actually setup to deal with UniCode strings in
 * addition to straight ASCII. In order to supply a UniCode string to
 * such routines, the UniCode string must start with a single byte value
 * of UNICODE_INTRODUCER. This tells the string parsing code that the
 * remainder of the string should be treated as a UniCode string and not
 * as ASCII.
 *
 * You can use these macro to help with this scheme
 */

/* the special byte to introduce a UniCode string */
#define UNICODE_INTRODUCER '\001'

/* a macro to tell you if a string is a UniCode string or not */
#define IS_UNICODE(s) ((s)[0] == UNICODE_INTRODUCER)
...
{
    OpeningItemNode   loc;                   /* system linkage */
