...
#ifndef lint
static char rcsid[] = "$Header: /user6/ouster/tcl/RCS/tclUnixAZ.c,v 1.30 91/11/04 09:56:13 ouster Exp $ SPRITE (Berkeley)";
#endif /* not lint */
    ...
    char **argv;			/* Argument strings. */
{
	...
	sprintf(string, "%d", statBuf.st_dev);
	if (Tcl_SetVar2(interp, argv[3], "dev", string, 0) == NULL) {
	    setError:
	    Tcl_AppendResult(interp,
		    "couldn't store stat information in variable \"",
		    argv[3], "\"", (char *) NULL);
	    return TCL_ERROR;
	}
	sprintf(string, "%d", statBuf.st_ino);
	if (Tcl_SetVar2(interp, argv[3], "ino", string, 0) == NULL) {
	    goto setError;
	}
	sprintf(string, "%d", statBuf.st_mode);
	if (Tcl_SetVar2(interp, argv[3], "mode", string, 0) == NULL) {
	    goto setError;
	}
	sprintf(string, "%d", statBuf.st_nlink);
	if (Tcl_SetVar2(interp, argv[3], "nlink", string, 0) == NULL) {
	    goto setError;
	}
	sprintf(string, "%d", statBuf.st_uid);
	if (Tcl_SetVar2(interp, argv[3], "uid", string, 0) == NULL) {
	    goto setError;
	}
	sprintf(string, "%d", statBuf.st_gid);
	if (Tcl_SetVar2(interp, argv[3], "gid", string, 0) == NULL) {
	    goto setError;
	}
	sprintf(string, "%ld", statBuf.st_size);
	if (Tcl_SetVar2(interp, argv[3], "size", string, 0) == NULL) {
	    goto setError;
	}
	sprintf(string, "%ld", statBuf.st_atime);
	if (Tcl_SetVar2(interp, argv[3], "atime", string, 0) == NULL) {
	    goto setError;
	}
	sprintf(string, "%ld", statBuf.st_mtime);
	if (Tcl_SetVar2(interp, argv[3], "mtime", string, 0) == NULL) {
	    goto setError;
	}
	sprintf(string, "%ld", statBuf.st_ctime);
	if (Tcl_SetVar2(interp, argv[3], "ctime", string, 0) == NULL) {
	    goto setError;
	}
	return TCL_OK;
    } else {
	Tcl_AppendResult(interp, "bad option \"", argv[1],
		...
		"extension, isdirectory, isfile, mtime, owned, ",
		"readable, root, size, stat, tail, or writable",
		(char *) NULL);
	return TCL_ERROR;
    }
    if (stat(fileName, &statBuf) == -1) {
