...
typedef Err (* IFFOpenFunc)(IFFParser *iff, void *openKey, bool writeMode);
typedef Err (* IFFCloseFunc)(IFFParser *iff);
typedef int32 (* IFFReadFunc)(IFFParser *iff, void *buffer, uint32 numBytes);
typedef int32 (* IFFWriteFunc)(IFFParser *iff, const void *buffer, uint32 numBytes);
typedef int32 (* IFFSeekFunc)(IFFParser *iff, int32 position);
...
typedef enum IFFParserTags
{
    ...
    IFF_TAG_FILE_MAC,                  /* FSSpec of file to parse */
#else
    IFF_TAG_IOFUNCS,                 /* callback functions to do IO       */
#endif
    IFF_TAG_IOFUNCS_DATA,            /* openKey parameter for IFFOpenFunc */
   IFF_TAG_MEMORY,                  /* memory location of data to parse */
    IFF_TAG_MEMORY_SIZE             /* size of memory buffer to parse   */} IFFParserTags;
...
int32 ReadChunkCompressed(IFFParser *iff, void *buffer, uint32 numBytes);
int32 WriteChunk(IFFParser *iff, const void *buffer, uint32 numBytes);
int32 WriteChunkCompressed(IFFParser *iff, const void *buffer, uint32 numBytes);
...
#endif  /* __cplusplus */
/* convenience */
void ConvertIDtoStr(PackedID id, char *result);
