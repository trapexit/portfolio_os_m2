/*
	...
	Change History (most recent first):
		...
		<4+>	 9/18/95	TMA		Folio bug, Callback
		 <4>	 7/15/95	TMA		Update to match new folio version.
		<1+>	 7/11/95	TMA		Fix path names for braindead Macs.
...
#ifdef applec 
static void FreeMem(void *p, int32 size)
{
  free(p);
}
#endif
...
/* TMA-For Container Chunk */
static void
IFFSwabShort(uint16* wp)
{
  register uint8  *cp = (uint8 *)wp;
  int t;
...
IFFSwabLongLong(uint64* lp)
{
  ...
  for (i=0; i<quit; i++, j--)
    {
      t = cp[j]; cp[j] = cp[i]; cp[i] = t;
    }
}

#define	ID_Container       MAKE_ID('\0','\0','\0','{')
#define IsContainerID(id) ((id & ID_Container) == ID_Container)
...
#ifdef INTEL
        IFFSwabLong(&(smallChunk.ID));
#endif
        chunk.ID   = smallChunk.ID;
#ifdef INTEL
        IFFSwabLong(&(smallChunk.Size));
#endif
...
{
    /* FIXME: Make this do compression */
    return ReadChunk(iff, buf, numBytes);
}
...
uint64       pos;
    ...
    switch (mode) {
        ...
        /* clip position in range of -base..cn_Size-base */
        /* result in uint64 newOffset in range of 0..cn_Size */
    if (position >= 0) {
        pos = position;
        ...
        /* seek on stream */
    {
            /* FIXME: fails when abs(newOffset - top->cn_Offset) >= 2^31 */
        const int32 seekOffset = (int32)(newOffset - top->cn_Offset);
        int32 oldStreamPos, newStreamPos;

            /* seek relative to current file position, and see if we got there
            ** (traps seek past end of file, which isn't a SeekRawFile() error) */
...
bool         firstChunk;
    ...
    else
    {
      ...
      /* TMA- Container */
        if ((cn->cn_ID != ID_FORM) && (cn->cn_ID != ID_PROP) && 
...
#ifdef INTEL
	    IFFSwabLongLong(&(chunk.Size));
		IFFSwabLong(&(chunk.ID));
		IFFSwabLong(&(chunk.Filler));
#endif
        if (firstChunk)
        {
	...
	cn->cn_Type = 0;   /*TMA - We need something to denote a Container */
    }
    else
    {
...
uint32       size32;
  ...
  uint32       alignment, padding, extra;
    ...
    if (unknownSize)
    {
      if (bit64)
	{
	  err = IFFSeek(iff, -(rsize + (int32)sizeof(size)));
	  if (err >= 0)
	    {
#ifdef INTEL
	    IFFSwabLongLong(&size);
#endif
      ...
      else
	{
	  ...
	  err = IFFSeek(iff, -(rsize + (int32)sizeof(size32)));
	  if (err >= 0)
	    {
#ifdef INTEL
	      IFFSwabLong(&size32);
...
{
    /* FIXME: Make this do compression */
    return WriteChunk(iff, buf, numBytes);
}
...
#ifdef applec
            case IFF_TAG_FILE_MAC   : openKey = tag->ta_Arg;
	                              ioFuncs = &fileFuncsMac;
	                              break;
#endif
            case IFF_TAG_MEMORY : openKey = tag->ta_Arg;
                                  ioFuncs = &memFuncs;
                                  break;
...
PackedID     topid;
    ...
    if (iff->iff_NewIO)
    {
        ...
        if ((top->cn_ID != ID_FORM)
         && (top->cn_ID != ID_CAT)
	 && (top->cn_ID != ID_LIST)
	  && (!IsContainerID(top->cn_ID)))      /* TMA- Container stuff */
            return IFF_ERR_NOTIFF;
