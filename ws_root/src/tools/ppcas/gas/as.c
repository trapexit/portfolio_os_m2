...
-a[sub-option...]	turn on listings\n\
  ...
  fprintf (stream, "\
-D SYM=VAL		define symbol SYM to given value\n\
...
/* to fix a bug with --defsym, we need to open the output file */
/* before processing any defsyms.  So find it and open it first, */
/* then call parse_args() to rescan the arguments */

#ifdef BFD_ASSEMBLER
void parse_args_for_output( int argc, char *argv[] )
{
    int i;
...
#if 0	/* 3DO Modification: -D == --defsym */
	case 'D':
...
#ifdef BFD_ASSEMBLER
  parse_args_for_output( argc, argv );
