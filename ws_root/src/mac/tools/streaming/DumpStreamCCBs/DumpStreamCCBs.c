...
NamedBit gCCBFlagBits[] = {
		"CCB_SKIP",			0x80000000,
		"CCB_LAST",			0x40000000,
		"CCB_NPABS",		0x20000000,
		"CCB_SPABS",		0x10000000,
		"CCB_PPABS",		0x08000000,
		"CCB_LDSIZE",		0x04000000,
		"CCB_LDPRS",		0x02000000,
		"CCB_LDPIXC",		0x01000000,
		...
		"CCB_LCE",			0x00008000,
		"CCB_ACE",			0x00004000,
		"CCB_reserved13",	0x00002000,
		"CCB_MARIA",		0x00001000,
		"CCB_PXOR",			0x00000800,
		"CCB_USEAV",		0x00000400,
		"CCB_PACKED",		0x00000200,
		"CCB_PLUTPOS",		0x00000040,
		"CCB_BGND",			0x00000020,
...
NamedBit gAVFlagBits[] = {
		"Disable output wrap prevention",	0x0004,
 ...
 *******************************************************************************************/
static void		DumpPIXC( char* label, unsigned long flags, unsigned long ccbFlags )
	{
	NamedBitPtr	pnb;
	short		outCount;
	short		flagNumber;
	...
	switch ( (flags >> 13) & 3 )
		{
		case 0:	printf( "00 - PMV from CCB value\n" );							break;
		case 1:	printf( "01 - PMV is AMV from decoder\n" );						break;
		case 2:	printf( "10 - PMV and PDV from color value out of decoder\n" );	break;
		case 3:	printf( "11 - PMV alone from color value out of decoder\n" );	break;
		}

	/* The MF field is only meaningful if the MS field is == 0 */
	if ( ((flags >> 13) & 3) == 0 )
		printf( "%s\t%s MF = %d (multiplier)\n", gCCBDetailsIndent, label, 1 + ((flags >> 10) & 7) );
	...
	switch ( (flags >> 6) & 3 )
		{
		case 0:	printf( "00 - none\n" );							break;
		case 1:	printf( "AV = %d\n", ((flags >> 1) & 0x1f) );		break;
		case 2:	printf( "10 - current frame buffer pixel\n" );		break;
		case 3:	printf( "11 - pixel coming from the decoder\n" );	break;
		}
...
#define	CCB_USEAV 	0x00000400
	...
	if ( ccbFlags & CCB_USEAV )
		...
		switch ( (flagsCopy >> 3) & 3 )
			...
			case 1:	printf( "2\n" );								break;
			case 2:	printf( "4\n" );								break;
			case 3:	printf( "2 LSB's of color from decoder\n" );	break;
			}
		...
		/* Secondary divider */
		printf( "%s\t%s Secondary divider = %d\n", gCCBDetailsIndent, label, 1 + (flags & 1) );
	}
 ...
 *******************************************************************************************/
static void		DumpSACCB( long filePosition, StreamAnimCCBPtr saccb )
	{
...
#define CCB_POVER_SHIFT  7
	...
	switch ( (flags & CCB_POVER_MASK) >> CCB_POVER_SHIFT )
		{
		case 0:		printf( "in pixel data\n" );	break;
		case 1:		printf( "**invalid**\n" );		break;
		case 2:		printf( "P-Mode 0\n" );			break;
		case 3:		printf( "P-Mode 1\n" );			break;
		}

	printf( "%sccb_X:\t\t%s\n", gCCBDetailsIndent, ConvertF16_String( buf, saccb->ccb_X ) );
	printf( "%sccb_Y:\t\t%s\n", gCCBDetailsIndent, ConvertF16_String( buf, saccb->ccb_Y ) );
...
#define CCB_LDPPMP      0x01000000
	if ( saccb->ccb_Flags & CCB_LDPPMP )
		...
		DumpPIXC( "P-mode 1: ", (saccb->ccb_PPMPC >> 16) & 0xffff, saccb->ccb_Flags );
		printf( "\n" );
		}
 ...
 *******************************************************************************************/
static void		DumpSAPOS( long filePosition, StreamAnimPosPtr sapos )
	{
	char	buf[64];
...
static void		DumpStreamFile( char* fileName, FILE* fd, long startTime )
	...
	while ( 1 == fread( buffer, sizeof(SubsChunkData), 1, fd ) )
		...
		if ( gBlockSize != 0 )
			 ...
			 * aligned on fixed blocks.
			 */
			nextBlockStart = ( (filePosition / gBlockSize) * gBlockSize ) + gBlockSize;
		...
		if ( fDumping || gCelFileFlag )
			...
			if ( NOT_QUADBYTE_ALIGNED( chunkPtr->chunkSize ) )
				printf( "Warning: following chunk size not a quadbyte multiple:\n" );
			...
			if ( chunkPtr->chunkType == SANM_CHUNK_TYPE )
				...
				if ( gDumpACCBFlag && ( saccb->subChunkType == ACCB_CHUNK_TYPE) )
					{
					if ( 1 != fread( buffer + sizeof(SubsChunkData), 
								sizeof(StreamAnimCCB) - sizeof(SubsChunkData), 1, fd ) )
						return;
				...
				else if ( gDumpAPOSFlag && (saccb->subChunkType == APOS_CHUNK_TYPE) )
					{
					if ( 1 != fread( buffer + sizeof(SubsChunkData), 
								sizeof(StreamAnimPos) - sizeof(SubsChunkData), 1, fd ) )
						return;
				...
				else if ( gDumpAMAPFlag && (saccb->subChunkType == AMAP_CHUNK_TYPE) )
					{
					if ( 1 != fread( buffer + sizeof(SubsChunkData), 
								sizeof(StreamAnimMap) - sizeof(SubsChunkData), 1, fd ) )
						return;
			 ...
			 * code will work correctly because we're set up to deal with streams,
			 * not old fashioned data files.
			 */
				 ...
				 * chunk type and chunk size fields, each of which are 4 byte fields.
				 */
				fseek( fd, filePosition + 4 + 4, SEEK_SET );
				if ( 1 == fread( &saccb->ccb_Flags, sizeof(StreamAnimCCB) - sizeof(SubsChunkData), 1, fd ) )
					{
