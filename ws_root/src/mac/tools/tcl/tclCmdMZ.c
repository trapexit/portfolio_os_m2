...
#ifndef lint
static char rcsid[] = "$Header: /user6/ouster/tcl/RCS/tclCmdMZ.c,v 1.12 91/10/27 16:17:07 ouster Exp $ SPRITE (Berkeley)";
#endif
    ...
    char **argv;			/* Argument strings. */
{
	...
	while ((clientData = Tcl_VarTraceInfo(interp, argv[2], 0,
	    ...
	    *p = '\0';
	    ...
	    tvarPtr->command[tvarPtr->length] = ' ';
	    prefix = " {";
	}
    } else {
	Tcl_AppendResult(interp, "bad option \"", argv[1],
    ...
    char **argv;			/* Argument strings. */
{
    ...
    while (1)
		{
		if (Tcl_User_Wants_Break(interp))
			{
			result = TCL_ERROR;
			break;
			}
