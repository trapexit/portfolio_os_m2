...
#ifndef lint
static char rcsid[] = "$Header: /user6/ouster/tcl/RCS/tclUnixAZ.c,v 1.33 92/01/04 15:25:41 ouster Exp $ SPRITE (Berkeley)";
#endif /* not lint */
...
/*
** The following routines allow you to "capture" standard "output"
** from the tcl output functions (puts stdout, etc...). This allows
** you to put them someplace other than a file or the "terminal".
** For example, "tickle" uses these routines to capture output and
** display it in the Macintosh text window in which the script is
** being executed.
**
** 
*/
...
static PFI	tcl_print_procedure = (PFI)0;

/*
** Sets the current "print proc". The print proc must be a
** function that will be called each time output is attempted
** from within the tcl interpreter. The code in the tcl interpreter
** looks like this:
**
**		if ( tcl_print_procedure != NULL &&
**				(filePtr->f == stderr || filePtr->f == stdout) )
**			{
**			(* tcl_print_procedure)(argv[2]);
**			}
**		else
**			{
**			... the standard tcl output code ...
**			}
**
** Therefore, your output proc should look like this:
**
**		my_print_proc(output_str)
**		char	*output_string;
**			{
**			... code to output/store the C-string "output_str" ...
**			}
**
** And your code should have initialization code that look like:
**
**		save_proc = Tcl_SetPrintProcedure(my_print_proc);
**
** NOTE: That according to the logic above, if you set the print proc to
**       "NULL", then stdout and stderr will be used as they normally
**       would. Thus, "Tcl_SetPrintProcedure(NULL)" will "reset" the output.
**
** The returned value "save_proc" is important if you want to
** re-direct the output only temporarily. To reset the output
** to the original output procedure later, use:
**
**		Tcl_SetPrintProcedure(save_proc);
*/

PFI
Tcl_SetPrintProcedure(proc)
PFI		proc;
{
PFI		result;

	result = tcl_print_procedure;
	tcl_print_procedure = proc;
	return result;
	}

/*
** This routine returns the procedure pointer to the current
** output proc. This allows you to compare to see if the output
** proc is what you want. For instance:
**
**		current_print_proc = Tcl_GetPrintProcedure();
**		if (current_print_proc != my_print_proc)
**			{
**			Tcl_SetPrintProcedure(my_print_proc);
**			}
**
**		... tcl scripting ...
**
**		Tcl_SetPrintProcedure(current_print_proc);
**
*/

PFI
Tcl_GetPrintProcedure()
{
	return tcl_print_procedure;
	}
...
{
char *p;
int length, statOp, accessResult;
...
char *fileName, c;
    ...
    if ((c == 'r') && (strncmp(argv[1], "readable", length) == 0)
		...
		mode = R_OK;
checkAccess:
		accessResult = access(fileName, mode);
		if (accessResult == -1) {
			interp->result = "0";
			}
		else {
    ...
    if ((c == 'a') && (strncmp(argv[1], "atime", length) == 0)) {
		...
		sprintf(interp->result, "%lu", statBuf.st_atime);
		return TCL_OK;
		}
		...
		sprintf(interp->result, "%lu", statBuf.st_mtime);
		return TCL_OK;
		}
		...
		sprintf(interp->result, "%lu", statBuf.st_size);
		return TCL_OK;
		}
					 ...
					 * stat data to store in varName. */
{
    ...
    sprintf(string, "%lu", statPtr->st_size);
    if (Tcl_SetVar2(interp, varName, "size", string, TCL_LEAVE_ERR_MSG)
	    == NULL) {
	return TCL_ERROR;
    }
    sprintf(string, "%lu", statPtr->st_atime);
    if (Tcl_SetVar2(interp, varName, "atime", string, TCL_LEAVE_ERR_MSG)
	    == NULL) {
	return TCL_ERROR;
    }
    sprintf(string, "%lu", statPtr->st_mtime);
    if (Tcl_SetVar2(interp, varName, "mtime", string, TCL_LEAVE_ERR_MSG)
	    == NULL) {
	return TCL_ERROR;
    }
    sprintf(string, "%lu", statPtr->st_ctime);
...
#   define BUF_SIZE 200
	...
	*p = 0;
	if (argc == 2) {
	    ...
	    Tcl_SetVar(interp, argv[2], buffer, flags);
    ...
    char **argv;			/* Argument strings. */
{
	...
	if (filePtr->f == NULL) {
		...
		Tcl_AppendResult(interp, "can not pipe in macintosh version", (char *) NULL);
		goto error;
#ifndef macintosh
    ...
    char **argv;			/* Argument strings. */
{
	...
	if (Tcl_GetInt(interp, argv[2], &numBytes) != TCL_OK) {
	    return TCL_ERROR;
	}
    } else {

	/*
	 * Compute how many bytes are left in the file.  Try to read
	 * one more byte than this, just to force the eof condition
	 * to be seen.
	 */

	if (fstat(fileno(filePtr->f), &statBuf) < 0) {
	    Tcl_AppendResult(interp,
		    "couldn't compute size of \"", argv[1],
		    "\": ", Tcl_UnixError(interp), (char *) NULL);
	    return TCL_ERROR;
	}
	numBytes = statBuf.st_size - ftell(filePtr->f) + 1;
	if (argc > 2) {
    ...
    /*
     * Read the bytes into a dynamically-allocated array, and
     * return it as result.
     */

    interp->result = (char *) ckalloc((unsigned) numBytes+1);
    interp->freeProc = (Tcl_FreeProc *) free;
    count = fread(interp->result, 1, numBytes, filePtr->f);
    if (ferror(filePtr->f)) {
	Tcl_ResetResult(interp);
	Tcl_AppendResult(interp, "error reading \"", argv[1],
		"\": ", Tcl_UnixError(interp), (char *) NULL);
    ...
    if ((newline == 0) && (interp->result[count-1] == '\n')) {
	interp->result[count-1] = 0;
    } else {
    ...
    char **argv;			/* Argument strings. */
{
    ...
    Tcl_ResetResult(interp);
#ifdef macintosh
    sprintf(interp->result, "%.0lf ticks per iteration", timePer / count);
#else
    sprintf(interp->result, "%.0lf microseconds per iteration", timePer/count);
#endif
    return TCL_OK;
				 ...
				 * there isn't any stderr output. */
{
	...
	Tcl_AppendResult(interp, " cleanup is unimplemented", (char *) NULL);
	return TCL_ERROR;
