...
void			tcl_setenv _ANSI_ARGS_((char *name, char *value));
void			tcl_unsetenv _ANSI_ARGS_((char *name));

/*
 *----------------------------------------------------------------------
 *
    ...
    */
    ...
    if (index == -1) {
		if ((length+2) > environSize) {
			...
			newEnviron = (char **) ckalloc((unsigned) ((length+5) * sizeof(char *)));
			memcpy((VOID *) newEnviron, (VOID *) environ, length*sizeof(char *));
			ckfree((char *) environ);
			environ = newEnviron;
    ...
    char *name;			/* Name of variable to remove. */
{
    ...
    for (eiPtr = firstInterpPtr; eiPtr != NULL; eiPtr = eiPtr->nextPtr) {
		(void) Tcl_UnsetVar2(eiPtr->interp, "env", name, TCL_GLOBAL_ONLY);
   		}
...
int flags;				/* Indicates what's happening. */
{
    ...
    */
    ...
    /*
     * If a value is being set, call tcl_setenv to do all of the work.
     */
