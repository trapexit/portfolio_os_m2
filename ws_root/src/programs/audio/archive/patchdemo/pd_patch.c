...
/******************************************************************************
**
**  @(#) pd_patch.c 95/10/30 1.6
**  $Id: pd_patch.c,v 1.19 1994/10/06 19:20:01 peabody Exp $
**
**  patchdemo patch structure management and file parser. See patchdemo.c
**  for documentation.
**
******************************************************************************/
...
/* !!! revise this module to use <audio/musicscript.h> instead of locally defined parser */

typedef struct ParserFile {
        /* constants */
    const char *pfile_FileName;     /* file name */
    RawFile *pfile_Stream;           /* stream associated with file */
...
static Err readpatchfile (const char *filename, Patch *patch)
{
    ...
    Err errcode = 0;
        ...
        /* open file */
    if ((file = OpenParserFile (filename, &errcode)) == NULL) goto clean;
...
static ParserFile *OpenParserFile (const char *filename, Err *errbuf)
{
    ...
    parserfile->pfile_FileName = filename;
        ...
        /* open file */
    errcode = OpenRawFile(&parserfile->pfile_Stream,filename, FILEOPEN_READ);
    if (errcode < 0) {
        goto clean;
    }
...
{
    if (parserfile) {
        CloseRes (CloseRawFile, parserfile->pfile_Stream);
        myfree (parserfile);
    }
...
/*
    ...
    SYNOPSIS
        ...
        int32 GetStreamLine (RawFile *stream, char *buf, int32 bufsize)
    ...
    ARGUMENTS
        ...
        stream      Stream created by OpenRawFile().
...
static int32 GetStreamLine (RawFile *stream, char *buf, int32 bufsize)
{
    ...
    while (n--) {
        if ((actual = ReadRawFile (stream, s, 1)) < 0) return actual;    /* read character to buf, return on error */
