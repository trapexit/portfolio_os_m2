...
OverlayTypes	ovlType;
	...
	{ 
    	...
    	case ET_FILE		: ovlType = OVL_CONFIRMDELETEFILE;	break;
    	case ET_DIRECTORY	: ovlType = OVL_CONFIRMDELETEDIR;	break;
    	case ET_FILESYSTEM	: ovlType = OVL_CONFIRMDELETEFSYS;	break;
	}

    result = PrepOverlay(req, &overlay, ovlType,
                         deleteControl->ctrl_X + (deleteControl->ctrl_Width / 2),
                         deleteControl->ctrl_Y + (deleteControl->ctrl_Height / 2),
                         en->en_Name);
    if (result < 0)
        return result;
        ...
        if (req->sr_RenderSig & sigs)
        {
            ...
            if (overlay.ov_Stabilized)
            {
                ...
                else
                {
                    ...
                    PrepControl(req, &okButton, buttonX - buttonWidth, buttonY, CTRL_TYPE_OVL_DELETE);
    ...
    MoveOverlay(req, &overlay, deleteControl->ctrl_X + (deleteControl->ctrl_Width / 2),
                               deleteControl->ctrl_Y + (deleteControl->ctrl_Height / 2));
