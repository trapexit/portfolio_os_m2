...
#define kCmdListBufSize     160000	/* 150000 words = 600k per list */
#else
#define kNumCmdListBufs     2
#define kCmdListBufSize     1024*16
#endif
...
Err GraphicsEnv_Init(GraphicsEnv* genv, uint32 width, uint32 height, uint32 depth)
{
    ...
    if (err < 0) {
				/*satoru*/		printf("Fail to open Graphics Folio\n");
	retVal = err;
	goto Failed_OpenGfx;
    }
    ...
    if (genv->d == NULL) {
				/*satoru*/		printf("Fail to create Display\n");
	retVal = Hg_NoMem;
    ...
    if (genv->d->signal == 0) {
		/*satoru*/		printf("Fail to Allocate signals\n");
	retVal = -1;
	goto Failed_AllocSignal;
    }
    err = Display_CreateView(genv->d);
    if (err < 0) {
		/*satoru*/		printf("Fail to create View\n");
	retVal = err;
    ...
    if (err < 0) {
				/*satoru*/		printf("Fail to allocate bitmap\n");
	retVal = err;
    ...
    err = ModifyGraphicsItemVA(genv->d->view,
			       ...
			       TAG_END);
    if (err < 0) {
				/*satoru*/		printf("Fail to modify Graphics Item\n");
		PrintfSysErr(err);
	}
    ...
    if (err < 0) {
		/*satoru*/		printf("Fail to allocate list\n");
	retVal = err;
