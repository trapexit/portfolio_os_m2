...
#				-Xstrict-ansi		# (Self-evident.  Won't allow // comments!)*
				-Xansi				# (Self-evident.  Will allow // comments!)*
				-Xunsigned-char		# Treat the char type as unsigned.*
				{DEBUG_OPTIONS} *
				-Xforce-prototypes	# Compiler will complain about functions without 'em*
				-Xlint=0x10			# All warnings except unused variable/function.  Add more
bits (from 0xFE) for fewer warnings.*
				-Xtrace-table=0		# Don't generate traceback tables.  This is like macsbug
names for you macheads
...
LIBS		= -lblitter_utils -l3do_scaf -lsubscriber -lds -ldsutils -lEZFlixDecoder
-lmusic  *
				-lclt -lspmath *
				-leventbroker -lc
...
BOOTDIR	= "{VECTORDIR}"
### CASE 2.0:
#MODULEDIR	= Fort:3DO:3DO_OS:M2_3.0d:Remote:System.m2:Modules:
#BOOTDIR	= Fort:3DO:3DO_OS:M2_3.0d:Remote:System.m2:Boot:
### } END SWITCH

MODULES		=	"{MODULEDIR}"blitter "{MODULEDIR}"audio "{MODULEDIR}"font *
...
{appname} ff {appname}.make {OBJECTS}
	...
	link3do			# NOTE: Type Link3DO and hit enter for full list of options *
		...
		-Htime=now -Hsubsys=1 -Hname={appname} -Htype=5 -Hstack=32768 # 3DO Header
Information  *
		-o {appname} *
		{OBJECTS} *
	...
	Set Echo 1
	setfile {appname} -c '3DOD' -t 'PROJ'				# Set creator and type so icon will
appear
	duplicate {appname} -y {3doremote}Pac_Man:			# copy the goodies to /remote
	Set Echo 0
...
at build time.
	echo "setsourcedir" > "{3doremote}{appname}.spt"					# Clear 3DODebug's
current list.
	echo "setsourcedir `directory`" >> "{3doremote}{appname}.spt"	# Tell 3DODebug
to look for source files in the current directory.
	#·· <<If you have more than one source directory, insert more lines here.>>
	echo "setdatadir" >> "{3doremote}{appname}.spt"						# Clear 3DODebug's
current list of symbol files.
	echo "setdatadir *"{3doremote}*"" >> "{3doremote}{appname}.spt"		# (Because
currently the symbols are in the executable file).
...
.c.o f .c Mpeg.make
	...
	if {USE_PPCAS}
		Set Echo 1
		dcc {depDir}{default}.c {defines} {dccopts} -Xdebug-mode=6 -S -o
"{TempFolder}"{default}.s
...
# .cp.o files are dependent only upon the .cp source (This is not very
...
.cp.o	 f .cp Mpeg.make
	...
	if {USE_PPCAS}
		Set Echo 1
		dplus {depDir}{default}.cp {defines} {dplusopts} -Xdebug-mode=6 -S -o
"{TempFolder}"{default}.s
...
# .cpp.o files are dependent only upon the .cpp source (This is not very
complete but good enough for now)
.cpp.o f .cpp Mpeg.make
	Set Echo 0
	Echo "# ---- Compiling {depDir}{default}.cpp"
	if {USE_PPCAS}
		Set Echo 1
		dplus {depDir}{default}.cpp {defines} {dplusopts} -Xdebug-mode=6 -S -o
"{TempFolder}"{default}.s
		...
		Set Echo 1
		dplus {depDir}{default}.cpp {defines} {dplusopts} -o
{targDir}{default}.cpp.o
		Set Echo 0
	end if
		Set Echo 1

# .s.o files are dependent only upon the .s source (This is not very complete
but good enough for now)
.s.o f .s Mpeg.make
	Set Echo 0
