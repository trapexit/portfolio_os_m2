...
/* set of 3 Frame Buffers plus a Z Buffer in a small region of memory.  The	*/
/* purpose is to generate Bitmap Items for 'virtual' screens of size 1312	*/
...
/* screens can be allocated in the space of a single 1312x938 pixel region.	*/
/* In addition, only a small portion equivalent to four 320x298 Bitmaps is	*/
...
/* window can be offset into the 'virtual' bitmaps by up to 960 pixels in	*/
/* both the x and y directions thus providing true hardware clipping!		*/

void CreateClip3Display (ScreenContext *SCPtr)
{
	uint8	*Buffer;
	int32	BufferEntry, ReturnVal;
	...
	/* Repeat Bitmap Item creation for additional buffers. */
	for (BufferEntry = 1; BufferEntry < SCREENBUFFERS; BufferEntry++)
	{
		if ((SCPtr->sc_BitmapItems[BufferEntry] =
				CreateItemVA (MKNODEID (NST_GRAPHICS, GFX_BITMAP_NODE),
							BMTAG_WIDTH, VIRTUAL_WIDTH,
							...
							BMTAG_BUMPDIMS, TRUE,
							TAG_END)) < 0)
			ShutDownClip (CREATEBITMAP, SCPtr->sc_BitmapItems[BufferEntry]);

		/* Retrieve the values in the Bitmap structure selected */
		/* by the Graphics Folio.								*/ 
		if ((SCPtr->sc_Bitmaps[BufferEntry] = (Bitmap *)
				LookupItem (SCPtr->sc_BitmapItems[BufferEntry])) == NULL)
			ShutDownClip (LOOKUPITEM, SCPtr->sc_BitmapItems[BufferEntry]);

		/* Attach Bitmap to BitmapItem.  In this case, the memory has		*/
		/* already been allocated for Bitmap 0, so a pointer is offset		*/
		/* into this same region by a carefully selected number of pages.	*/
		/* This page offset is very important, because this is what keeps	*/
		/* alignments correct for the Z Buffer and also allows the			*/
		/* 'virtual' bitmaps to overlap.									*/
		if ((ReturnVal =
			ModifyGraphicsItemVA (SCPtr->sc_BitmapItems[BufferEntry],
				BMTAG_BUFFER, (Buffer + (PAGE_OFFSET * BufferEntry * PAGESIZE)),
				TAG_END)) < 0) ShutDownClip (MODIFYGFX, ReturnVal);
	}
	...
	SCPtr->sc_FreeZoneSize = VIRTUAL_WIDTH * Y_OFFSET * 2;
...
void DestroyClipDisplay (ScreenContext *SCPtr)
{
    ...
    if ((ReturnVal = FreeSignal (SCPtr->sc_DisplaySignal)) < 0)
		ShutDownClip (FREESIGNAL, ReturnVal);
